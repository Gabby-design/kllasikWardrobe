export async function sendBrevoEmail({ to, subject, htmlContent }) {
  ...
  if (!apiKey) {
    console.warn('⚠️ BREVO_API_KEY is not configured in .env.local. Skipping email sending.');
    return { skipped: true, message: 'BREVO_API_KEY not configured' };
  }

  if (!to || !Array.isArray(to) || to.length === 0 || !to[0]) {
    console.warn('⚠️ No recipient email provided. Skipping email sending.');
    return { skipped: true, message: 'No recipient email' };
  }
  ...
  try {
    ...
    console.error('⚠️ Brevo network error:', err);
